...
| BUG-01-home-route-db-localhost | Debugging: `GET /` returned 500 for every request during OOM-limit load testing | `k8s/app-deployment.yaml` (live cluster drift, not application code) | Found incidentally while load-testing `clinic-app` under an intentionally low memory limit (32Mi) to reproduce a real OOM crash for report evidence. `kubectl top` showed the pod using only ~20Mi, well under the limit, so memory pressure was ruled out; `kubectl logs` instead showed an `AggregateError [ECONNREFUSED]` from `pg-pool`, attempting to reach Postgres at `localhost:5432` rather than the cluster's `clinic-db` Service DNS name. Initial hypothesis (logged here at the time) was a code defect in `routes/home.js`; static review of `db/connection.js`, `k8s/configmap.yaml`, and `app-deployment.yaml` later showed all three were correct, `DB_HOST` was properly wired via `configMapKeyRef` in every committed file. Live verification (`kubectl exec ... env grep DB_`) then showed the running pod had no `DB_HOST` at all, despite the committed manifest defining it: the live deployment had drifted from the version in version control. `kubectl apply -f k8s/app-deployment.yaml` resynced live state to the committed manifest and fixed it immediately, confirmed via `DB_HOST=clinic-db` in the new pod's environment and a `200` (was `500`) on `GET /`. A second instance of this project's "verify, don't assume" theme within the theme itself: the first diagnosis (blaming application code) was itself an unverified assumption, corrected only by checking live cluster state rather than trusting the more obvious-looking explanation |
